...
public class GardenSolver implements Iterable<Choice> {
	/**
	 * the root of our circular linked list. This is the only Choice that is allowed to have Root as it's attribute, and is never returned for iterations.
	 */
	...
	public GardenSolver(int height, int width, double empty_prevalence_perc){
		...
		for(int y = 0; y < height; ++y){
			for(int x = 0; x < width; ++x){
				 ...
				 * 
				 * One Type, one Color, and one Size. (must have exactly one of each)
				 * Water (this precludes the piece having any other attribute)
				 * Empty (the "lack" of a piece, which is in itself a type of piece. this precludes the "piece" having any other attribute)
				 */
				...
				// by choosing one size, we cannot choose any other size at the same position
				Choice.linkExclusiveChoices(choices[6], choices[7]);
				
				// Whether the piece is water
				choices[8] = new Choice(Attribute.Water, pos);
				// If we choose water, the piece cannot have any Sizes, Colors, or Types.
				for(int i = 0; i < choices.length - 1; ++i)
					Choice.linkExclusiveChoices(choices[8], choices[i]); // link water to all other choices
				
				//Whether there is no piece at all.
				Choice empty = new Choice(Attribute.Empty, pos);
				// If we choose empty, we can choose no other attributes for this location.
				for(Choice c : choices)
					Choice.linkExclusiveChoices(empty, c); // link empty to all other choices
				
				// add all of our Choices to the relevant table, so we can look things up by position and attribute at any point.
				// We also need all our nodes to be in the HashTable for shuffle() to work
		...
		// shuffle the elements in the tables we added. Coincidentally, this also adds them to _root.
		shuffle(empty_prevalence_perc);
		
		// lock our choices. If we ever need to reset our graph, we can avoid having to create everything again.
	...
	private void shuffle(double empty_prevalence_perc){
		// as 10% of our values will always be Empty's, empty_prevalence_perc can't go below %10.
		if(empty_prevalence_perc < .1) empty_prevalence_perc = .1;
		else if(empty_prevalence_perc > 1) empty_prevalence_perc = 1;
		
		// temporary lists that hold our choices. Used so we can call Collections.shuffle on our Choices.
		// Choices listed as Empty need to be shuffled into the first part of our list, so we need to separate them now.
		LinkedList<Choice> temp_emptys = new LinkedList<Choice>();
		LinkedList<Choice> temp_choices = new LinkedList<Choice>();
		
		for(Map<Attribute, Choice> posmap : _choiceTable.values()){
			for(Choice choice : posmap.values()){
				// disconnect all choices from the list to avoid any weird adding problems, just in case.
				choice.close();
				// all Attribute.Empty's will get shuffled into the front part of the list, so set them aside for now. 
				if(choice.getAttribute() == Attribute.Empty) temp_emptys.add(choice);
				// everything else gets added to a general piece pool.
				else temp_choices.add(choice);
			}
		}
		
		// randomly choose enough items from the choices list so that when shuffled together with the emptys, they form the first %empty_prevalence_perc of our list
		Collections.shuffle(temp_choices);
		LinkedList<Choice> firstNitems = new LinkedList<Choice>();
		// (total_number_of_choices * %of_list_emptys_will_be_shuffled_into) - number_of_emptys = the number of non-empty choices we need to randomly grab.
		...
		// then merge our two lists - the one with the empty's first and add them back into _root.
		for(Choice c : firstNitems) _root.addChoice(c);
	...
	private boolean isValid(Position p){
		...
		if(!(pos.get(Attribute.Empty).isClosed() || pos.get(Attribute.Water).isClosed())) return true;
		
		// If both are closed, then we need to choose one Type, one Color, and one Size.
		//If all of a certain category is closed, we don't have a valid Space and whatever current Choice set we currently have is invalid.
		...
		if(typevalid && colorvalid && sizevalid) return true;
		else return false;
	}
	@Override
